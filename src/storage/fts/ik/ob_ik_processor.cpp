...
int TokenizeContext::init()
{
  ...
  if (OB_ISNULL(fulltext_) || fulltext_len_ <= 0 || OB_ISNULL(well_formed_len_)) {
...
int TokenizeContext::prepare_next_char()
{
  int ret = OB_SUCCESS;
  int error = 0;
  const char *ch = fulltext_ + cursor_;
  next_char_len_ = static_cast<int64_t>(
      well_formed_len_(charset_info_, ch, fulltext_ + fulltext_len_, 1, &error));
  if (OB_UNLIKELY(0 != error)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid encoding found", K(ret));
  } else if (OB_FAIL(ObFTCharUtil::classify_first_char(
                 coll_type_, ch, static_cast<uint8_t>(next_char_len_), next_char_type_))) {
