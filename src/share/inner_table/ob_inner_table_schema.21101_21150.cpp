...
int ObInnerTableSchema::connection_control_failed_login_attempts_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   select     concat('''',u.user_name,'''@''',u.host,'''') as USERHOST,     s.failed_login_attempts as FAILED_ATTEMPTS   from oceanbase.__all_user_failed_login_stat s   join oceanbase.__all_user u   on s.user_id = u.user_id )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
