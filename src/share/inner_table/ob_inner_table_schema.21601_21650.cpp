...
int ObInnerTableSchema::dba_ob_ccl_rules_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(           SELECT           CCL_RULE_ID,           CCL_RULE_NAME,           AFFECT_USER_NAME,           AFFECT_HOST,           AFFECT_FOR_ALL_DATABASES,           AFFECT_FOR_ALL_TABLES,           AFFECT_DATABASE,           AFFECT_TABLE,           AFFECT_DML,           AFFECT_SCOPE,           CCL_KEYWORDS,           MAX_CONCURRENCY         FROM oceanbase.__all_ccl_rule )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
