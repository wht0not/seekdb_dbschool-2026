...
int ObInnerTableSchema::time_zone_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT time_zone_id as Time_zone_id,            use_leap_seconds as Use_leap_seconds     FROM oceanbase.__all_time_zone )__"))) {
...
int ObInnerTableSchema::time_zone_name_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT name as Name,            time_zone_id as Time_zone_id     FROM oceanbase.__all_time_zone_name )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::time_zone_transition_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT time_zone_id as Time_zone_id,            transition_time as Transition_time,            transition_type_id as Transition_type_id     FROM oceanbase.__all_time_zone_transition )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::time_zone_transition_type_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT time_zone_id as Time_zone_id,            transition_type_id as Transition_type_id,            offset as Offset,            is_dst as Is_DST,            abbreviation as Abbreviation     FROM oceanbase.__all_time_zone_transition_type )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_session_longops_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT CAST(sid AS SIGNED) AS SID,            CAST(trace_id AS CHAR(64)) AS TRACE_ID,            CAST(op_name AS CHAR(64)) AS OPNAME,            CAST(TARGET AS CHAR(64)) AS TARGET,            CAST(USEC_TO_TIME(START_TIME) AS DATETIME) AS START_TIME,            CAST(ELAPSED_TIME/1000000 AS SIGNED) AS ELAPSED_SECONDS,            CAST(REMAINING_TIME AS SIGNED) AS TIME_REMAINING,            CAST(USEC_TO_TIME(LAST_UPDATE_TIME) AS DATETIME) AS LAST_UPDATE_TIME,            CAST(MESSAGE AS CHAR(512)) AS MESSAGE     FROM oceanbase.__all_virtual_long_ops_status )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
WHERE T.TABLE_ID = C.TABLE_ID
      ...
      AND 0 = sys_privilege_check('table_acc', 1, D.DATABASE_NAME, T.TABLE_NAME))__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
